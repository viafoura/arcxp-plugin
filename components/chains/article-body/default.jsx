...
import { CommentsComponent } from './_children/CommentsComponent';
...
function parseArticleItem(item, index, arcSite, phrases, id, customFields) {
  ...
  switch (type) {
    ...
    case 'divider': {
      return (
        <Divider assistiveHidden={false} key={`${type}_${index}_${key}`} />
      );
    }
            ...
            <Conditional component={Link} condition={link} href={link}>
              <Image
                {...getResizeParamsFromANSImage(
                  ...
                  [274, 400, 768, 1024, 1440].map((w) =>
                    allowedFloatValue ? w / 2 : w
                  )
                )}
                alt={altText}
              />
    ...
    case 'interstitial_link': {
      ...
      return (
        ...
        >
          ...
          <Link
            href={url}
            aria-label={phrases.t(
              'article-body-block.interstitial-link-aria-label'
            )}
          >
            {content}
          </Link>
    ...
    case 'list': {
      ...
      return listItems && listItems.length > 0 ? (
        <List
          key={`${type}_${index}_${listId}_${key}`}
          listType={listType}
          listItems={listItems}
        />
      ) : null;
    ...
    case 'correction': {
      ...
      return item.text && item.text.length > 0 ? (
        <section
          className={`${BLOCK_CLASS_NAME}__correction`}
          key={`${type}_${index}_${key}`}
        >
          <HeadingSection>
